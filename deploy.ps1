...
if (-not $SkipBuild -and -not $DryRun) {
    ...
    gcloud run deploy sentinel-nstech --source . --region us-central1 --project sinistroia --allow-unauthenticated --memory 2Gi --timeout 900s --set-env-vars "ASPNETCORE_ENVIRONMENT=Production,MediaProcessor__BaseUrl=https://sentinel-cortex-557004456190.us-central1.run.app"
